 ...
 $inter=new WP_Query($args);
 if($inter): ?>
     <div class="row">
         <?php
         $i=0;
         while ($inter->have_posts()):$inter->the_post(); $i++;?>
         ...
         endwhile;?><?php if ($autos->max_num_pages > 1) : // custom pagination  ?>
             <?php
             $orig_query = $wp_query; // fix for pagination to work
             $wp_query = $inter;
             $prev_link = get_previous_posts_link( '&laquo; '.__( 'Older Entries', 'iamsocial' ) );
             $next_link = get_next_posts_link( __( 'Newer Entries', 'iamsocial' ).' &raquo;',$inter->max_num_pages  );
         ...
         <div class="col-xs-12">
             ...
             <?php do_shortcode('[my-booking-form-internacional]') ?>
